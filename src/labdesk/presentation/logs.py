...
from .. import db
from . import tasks
from .widgets import muted, page_header, toast_info, toast_warn
...
class LogsPage(QWidget):
    ...
    def refresh(self) -> None:
        ...
        self.summary.setText(
            f"{n} entr{'y' if n == 1 else 'ies'} shown (newest first, max 1000)"
        )
    ...
    def clear_old(self) -> None:
        ...
        def work(con) -> bool:
            con.execute(
                "DELETE FROM audit_log WHERE at < datetime('now','localtime','-90 days')"
            )
            con.commit()
            db.log_audit(
        ...
        def done(work_ok: bool, result) -> None:
            if not work_ok:
                toast_warn(self, "Clear logs", f"Could not clear logs:\n{result}")
            self.refresh()
        ...
        tasks.run_in_background(
            ...
            clicked=self.clear_btn,
            lock=(self.verify_btn,),
            busy_text="Clearing…",
        )
