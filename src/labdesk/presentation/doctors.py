...
from .widgets import page_header, toast_warn
...
class DoctorDialog(QDialog):
    def __init__(self, parent: QWidget | None = None, data=None) -> None:
        ...
        self.area = QLineEdit()
        self.area.setMaxLength(40)
        self.tel = QLineEdit()
        self.tel.setMaxLength(20)
        self.mobile = QLineEdit()
        self.mobile.setMaxLength(20)
...
class DoctorsPage(QWidget):
    def __init__(self, con, user) -> None:
        ...
        header, _ = page_header(
            "Referring Doctors", "", add, self.edit_btn, self.del_btn
        )
    ...
    def edit(self) -> None:
        ...
        if d.exec() == QDialog.Accepted:
            v = d.values()
            if not v["name"]:
                toast_warn(self, "Doctor", "Name cannot be empty.")
                return
            self.con.execute(
    ...
    def delete(self) -> None:
        ...
        if not can(self.user["role"], "delete"):
            toast_warn(self, "Delete", "You don't have permission to delete doctors.")
            return
        if (
            QMessageBox.question(self, "Delete", "Delete this doctor?")
            == QMessageBox.Yes
        ):
            row = self.con.execute(
                "SELECT name FROM doctors WHERE id=?", (did,)
            ).fetchone()
