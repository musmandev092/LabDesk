...
# schema.sql / seed.sqlite live one level up, next to the labdesk package's
# other modules (this file is now inside the db/ subpackage).
SCHEMA_FILE = Path(__file__).resolve().parent.parent / "schema.sql"
SEED_DB = Path(__file__).resolve().parent.parent / "seed.sqlite"  # ships with the catalog
