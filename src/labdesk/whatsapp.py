...
def _post(cfg, path, payload, timeout=None):
    ...
    with urllib.request.urlopen(
        req, timeout=timeout or cfg.get("timeout", _TIMEOUT_POST)
    ) as r:
...
def _get(cfg, path, timeout=_TIMEOUT_GET):
    ...
    if not ok:
        raise ValueError(why)
    req = urllib.request.Request(
        f"{cfg['url']}{path}", headers=_headers(cfg), method="GET"
    )
...
def config_ready(con) -> tuple[bool, str]:
    ...
    if not cfg["url"]:
        return (
            False,
            "WhatsApp isn't set up yet. Add the Gateway URL in Settings → WhatsApp.",
        )
    if not cfg["token"]:
        return (
            False,
            "WhatsApp isn't set up yet. Add the Access token in Settings → WhatsApp.",
        )
    return True, ""
...
def recipient_ready(con, receipt_id: int) -> tuple[bool, str]:
    ...
    if "wa_optout" in row.keys() and row["wa_optout"]:
        return False, (
            "This patient hasn't agreed to receive WhatsApp messages "
...
def check_status(con) -> tuple[bool, str]:
    ...
    try:
        ...
        if data.get("connected"):
            return (
                False,
                "Gateway reachable, but WhatsApp isn't linked — scan the QR code.",
            )
        return (
            False,
            "Gateway reachable, but the session isn't connected. Start/scan it.",
        )
...
def send_pdf(
    ...
    document = "data:application/pdf;base64," + base64.b64encode(p.read_bytes()).decode(
        "ascii"
    )
    payload = {
        "Phone": phone,
    ...
    try:
        ...
        low = body.lower()
        not_linked = any(
            s in low for s in ("logged in", "loggedin", "no session", "not connected")
        )
        success = None
        if j is not None:
            if "success" in j:
