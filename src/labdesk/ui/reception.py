...
from .widgets import card, field_label, money, muted, page_header
...
class ReceptionPage(QWidget):
    ...
    def _request_discount_approval(self):
        ...
        if not approver or not roles.can(approver["role"], "apply_discount"):
            ...
            QMessageBox.warning(
                self, "Approval", "Invalid credentials, or that user can't approve discounts."
            )
            return
    ...
    def update_specimen_options(self):
        ...
        if ids:
            q = (
                "SELECT DISTINCT sample_required FROM tests WHERE id IN (%s) "
    ...
    def search_tests(self, text=None):
        ...
        if len(text) < 1:
            self.results.addItem(
                self._hint_item("Start typing a test name or number above to see matches…")
            )
            return
        like = f"%{text}%"
    ...
    def _show_panel_menu(self):
        ...
        if not panels:
            ...
            for p in panels:
                menu.addAction(
                    p["name"], lambda _=False, pid=p["id"], nm=p["name"]: self._add_panel(pid, nm)
                )
    ...
    def save(self, do_print=True):
        ...
        if self.discount.value() > self._promo_pct() + 1e-9 and not (
            ...
            QMessageBox.warning(
                self, "Discount", "A discount above the promo needs manager/admin approval."
            )
            return
        c = self.con
        ...
        try:
            if pid:  # returning patient → reuse row + permanent MR, refresh details
                ...
                pid = c.execute(
                    ...
                    (title, mr_no, name, age, age_desc, sex, tel, addr, optout, consent_at),
                ).lastrowid
                if not mr_no:
            ...
            like = serial_prefix.replace("\\", "\\\\").replace("%", "\\%").replace("_", "\\_") + "%"
            top = c.execute(
            ...
            for bump in range(500):
                ...
                try:
                    c.execute(
                        "UPDATE receipts SET lab_no=?, case_no=? WHERE id=?", (cand, cand, rid)
                    )
                    lab_no = cand
                    break
        ...
        if disc_pct > 0:  # always record any discount (promo, self-applied, or approved)
            who = self._discount_approved_by or (
                "promo" if disc_pct <= self._promo_pct() + 1e-9 else self.user["username"]
            )
            db.log_audit(
                c,
                self.user["username"],
