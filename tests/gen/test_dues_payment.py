...
def _ledger_state(t, rid):
    row = t.con.execute(
        ...
        "WHERE ref_id=? AND kind='due_recovery'",
        (rid,),
    ).fetchone()
...
def _audit_count(t, lab_no=None):
    if lab_no is None:
        return t.con.execute(
            "SELECT COUNT(*) FROM audit_log WHERE action='due_received'"
        ).fetchone()[0]
    return t.con.execute(
...
def register(t):
    ...
    for sub in SUBS:
        for frac in PAID_FRACS:
            ...
            if due0 > 0:
                AMTS = [due0 / 3, due0 / 2, due0, due0 + 1, due0 + 500, sub + 1000, 0, -1, -9999]
            else:
                AMTS = [1, 100, 0, -5]
            for amt in AMTS:
                ...
                row = t.con.execute(
                    "SELECT lab_no, net_amount, paid, due FROM receipts WHERE id=?", (rid,)
                ).fetchone()
                ...
                if not should_apply:
                    ...
                    t.check(res is None, f"no-op returns None sub={sub} paid={paid0} amt={amt}")
                    t.eq(row["paid"], paid0, f"no-op leaves paid sub={sub} paid={paid0} amt={amt}")
                    t.eq(row["due"], due0, f"no-op leaves due sub={sub} paid={paid0} amt={amt}")
                    t.eq(cur_cnt, base_cnt, f"no-op writes no ledger sub={sub} amt={amt}")
                    t.eq(cur_audit, base_audit, f"no-op writes no audit sub={sub} amt={amt}")
                    continue

                # applied path -------------------------------------------------
                exp_paid = round(paid0 + amt, 2)
                exp_due = round(max(0.0, sub - exp_paid), 2)
                t.check(res is not None, f"applied returns tuple sub={sub} paid={paid0} amt={amt}")
                t.eq(res[0], row["lab_no"], f"returns lab_no sub={sub} amt={amt}")
                t.near(res[1], exp_paid, f"returns new_paid sub={sub} paid={paid0} amt={amt}")
                t.near(res[2], exp_due, f"returns new_due sub={sub} paid={paid0} amt={amt}")
                t.near(row["paid"], exp_paid, f"paid accumulated sub={sub} paid={paid0} amt={amt}")
                t.near(row["due"], exp_due, f"due recomputed sub={sub} paid={paid0} amt={amt}")
                # core invariant: due never negative
                t.check(row["due"] >= -1e-9, f"due>=0 sub={sub} paid={paid0} amt={amt}")
                ...
                if amt >= due0 - 1e-9:
                    t.near(
                        row["due"], 0.0, f"overpay clamps due to 0 sub={sub} paid={paid0} amt={amt}"
                    )
                    if amt > due0 + 1e-9:
                        t.check(
                            row["paid"] > sub - 1e-9,
                    ...
                    t.check(row["due"] > 0, f"partial leaves due sub={sub} paid={paid0} amt={amt}")
                # exactly one ledger credit of `amt`
                t.eq(cur_cnt, base_cnt + 1, f"one ledger row sub={sub} amt={amt}")
                # ledger stores the RAW amount (no rounding in receive_due)
                t.near(
                    cur_credit - base_credit,
                    amt,
                    f"ledger credit == amt sub={sub} amt={amt}",
                    tol=1e-6,
                )
                # exactly one audit row appended
                t.eq(cur_audit, base_audit + 1, f"one audit row sub={sub} amt={amt}")
                # audit references this lab_no
                t.check(
                    _audit_count(t, row["lab_no"]) >= 1, f"audit names lab_no sub={sub} amt={amt}"
                )
    ...
    for sub in SEQ_SUBS:
        for splits in SPLITS:
            ...
            running_paid = 0.0
            for amt in splits:
                row0 = t.con.execute("SELECT due FROM receipts WHERE id=?", (rid,)).fetchone()
                due_before = row0["due"]
                res = t.db.receive_due(t.con, rid, amt, "tester")
                row = t.con.execute("SELECT paid, due FROM receipts WHERE id=?", (rid,)).fetchone()
                if due_before <= 0 or amt <= 0:
                    t.check(res is None, f"seq no-op None sub={sub} amt={amt}")
                    continue
                ...
                t.near(
                    row["due"],
                    round(max(0.0, sub - running_paid), 2),
                    f"seq due tracks sub={sub} splits={splits} amt={amt}",
                )
                t.check(row["due"] >= -1e-9, f"seq due>=0 sub={sub} splits={splits} amt={amt}")
                # monotonic: due never increases across a payment
                t.check(
                    row["due"] <= due_before + 1e-9,
                    f"seq due monotone sub={sub} splits={splits} amt={amt}",
                )
            # after a sequence whose sum >= sub, due must be zero
            if sum(splits) >= sub:
                final = t.con.execute("SELECT due FROM receipts WHERE id=?", (rid,)).fetchone()[
                    "due"
                ]
                t.near(final, 0.0, f"seq fully paid -> due 0 sub={sub} splits={splits}")
            ...
            t.eq(cnt, applied, f"seq ledger count == applied sub={sub} splits={splits}")
    ...
    for sub, amt in PREC:
        ...
        t.near(round(row["paid"], 2), row["paid"], f"prec paid 2dp sub={sub} amt={amt}")
        t.near(round(row["due"], 2), row["due"], f"prec due 2dp sub={sub} amt={amt}")
