...
def _income(con, NV, f, t):
    return con.execute(
        ...
        f"WHERE {NV} AND date(received_at) BETWEEN ? AND ?",
        (f, t),
    ).fetchone()[0]
...
def _expense(con, f, t):
    return con.execute(
        "SELECT COALESCE(SUM(amount),0) FROM expenses WHERE date BETWEEN ? AND ?", (f, t)
    ).fetchone()[0]
...
def _ins_receipt(con, lab, paid, due, net, voided, method, when):
    return con.execute(
        ...
        "received_at) VALUES (?,?,?,?,?,?,?)",
        (lab, paid, due, net, voided, method, when),
    ).lastrowid


def _ins_expense(con, amount, when):
    con.execute(
        "INSERT INTO expenses(date,head,detail,amount) VALUES (?,?,?,?)", (when, "h", "d", amount)
    )
...
def register(t):
    ...
    METHODS = [None, "", "  ", "Cash", "Card", "JazzCash", "EasyPaisa", " Card ", "\tBank\t"]
    VOIDS = [0, 1]
    ...
    for paid in PAIDS:
        for pm in METHODS:
            for voided in VOIDS:
                ...
                t.near(got_inc, exp_inc, f"income paid={paid} pm={pm!r} void={voided}")
                ...
                if voided == 0 and paid > 0:
                    ...
                    if rows:
                        r0 = rows[0]
                        t.eq(r0["m"], _norm_method(pm), f"method label pm={pm!r}")
                        t.eq(r0["n"], 1, f"method count pm={pm!r}")
                        t.near(r0["total"], paid, f"method total pm={pm!r}")
                else:
                    t.eq(len(rows), 0, f"no method row paid={paid} void={voided}")
    ...
    for mix in MIXES:
        ...
        for k, (paid, pm, voided) in enumerate(mix):
            net = paid + 100.0
            due = max(0.0, net - paid)
            _ins_receipt(con, f"MIX{i}_{k}", paid, due, net, voided, pm, day(i, 5 + k))
            if voided == 0:
                exp_income += paid
                if paid > 0:
                    m = _norm_method(pm)
        ...
        for a, b in zip(totals, totals[1:], strict=False):
    ...
    for k in range(60):
        ...
        for d in days:
            ...
            if 10 <= d <= 20:
                in_range += paid
        con.commit()
        t.near(_income(con, NV, f, tt), in_range, f"boundary income k={k} ({f}..{tt})")
        ...
        t.near(
            _income(con, NV, f"{yr:04d}-{mo:02d}-20", f"{yr:04d}-{mo:02d}-20"),
            120.0,
            f"single-day inclusive k={k}",
        )
    ...
    for due in DUES:
        for voided in (0, 1):
            ...
            net = due + 500.0
            paid = net - due
            _ins_receipt(con, f"DUE{i}", paid, due, net, voided, "Cash", day(i, 12))
            con.commit()
            ...
            if counts:
                ...
                added_rows += 1
            t.near(
                _due_all_time(con, NV),
                base_due + added,
                f"all-time due running due={due} void={voided}",
            )
            t.eq(
                len(_dues_rows(con, NV)),
                base_due_rows + added_rows,
    ...
    con.commit()
    t.near(_due_all_time(con, NV), base_due + added, "due==0.005 excluded (strict >)")
    i = scn
    scn += 1
    _ins_receipt(con, f"THR{i}B", 1.0, 0.0051, 1.0051, 0, "Cash", day(i, 1))
    con.commit()
    t.near(_due_all_time(con, NV), base_due + added + 0.0051, "due==0.0051 included")
    ...
    for exps in EXP_SETS:
        for inc in INC_SETS:
            ...
            t.near(got_exp, in_win_exp + window_extra, f"expense sum exps={exps!r} inc={inc}")
            t.near(got_inc, inc, f"net income exps={exps!r} inc={inc}")
            net = got_inc - got_exp
            t.near(
                net,
                inc - (in_win_exp + window_extra),
                f"net=income-expense exps={exps!r} inc={inc}",
            )
            # net sign classification matches accounts.py color rule (net<0)
            t.check((net < 0) == (got_inc < got_exp), f"net sign exps={exps!r} inc={inc}")
    ...
    for net, paid0, recover in SETTLE:
        ...
        rid = _ins_receipt(con, f"SET{i}", paid0, due0, net, 0, "Cash", day(i, 7))
        con.commit()
        before = _due_all_time(con, NV)
        res = t.db.receive_due(con, rid, recover, "tester")
        row = con.execute("SELECT paid, due FROM receipts WHERE id=?", (rid,)).fetchone()
        if due0 <= 0:
    ...
    for k in range(25):
        ...
        _ins_receipt(con, f"VOID{i}", 50000.0, 5000.0, 55000.0, 1, "Card", day(i, 9))
        con.commit()
        ...
        _ins_receipt(con, f"LIVE{i}", 123.0, 0.0, 123.0, 0, "Cash", day(i, 9))
        con.commit()
