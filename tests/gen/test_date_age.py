...
_AGE_LINE_RE = re.compile(r'Age / Sex</span><span class="v">([^<]*)</span>')
...
def register(t):
    ...
    t.eq(list(_CONST_AGE_UNITS), AGE_UNITS, "constants.AGE_UNITS == [Years,Months,Days]")
    ...
    for unit in AGE_UNITS:
        for sex in SEXES:
            for age in AGES:
                ...
                con.execute(
                    "UPDATE receipts SET age_desc=?, received_at=? WHERE id=?",
                    (unit, "2026-06-01 09:00", rid),
                )
                con.commit()
                rh, err = _safe_report_html(t, rid)
                t.check(
                    err is None, f"report html never raises age={age} {unit} sex={sex} err={err}"
                )
                if rh is None:
                    continue
                want = f"{age} {unit} / {sex}"
                line = _age_line(rh)
                t.eq(line, want, f"age line age={age} unit={unit} sex={sex}")
    ...
    for stored, shown in EDGE_UNITS:
        ...
        con.execute(
            "UPDATE receipts SET age_desc=?, received_at='2026-06-01 09:00' " "WHERE id=?",
            (stored, rid),
        )
        con.commit()
        rh, err = _safe_report_html(t, rid)
    ...
    for age in AGE_EXTREMES:
        ...
        try:
            ...
            rb, errb = None, repr(e)
        t.check(errb is None and _is_pdf(rb), f"report PDF builds for extreme age={age} err={errb}")
    ...
    for i in range(40):
        ...
        _stamp(rid)
        again = _get(rid)
        t.eq(again["reported_at"], first_stamp, f"reprint keeps original reported_at i={i}")
    ...
    for i in range(15):
        pid = con.execute(
            "INSERT INTO patients(name,age,age_desc,sex,telephone) "
            "VALUES('Prog',30,'Years','Male','03001234567')"
        ).lastrowid
        rid = con.execute(
            "INSERT INTO receipts(lab_no,patient_id,patient_name,age,age_desc,sex,"
            "subtotal,net_amount,paid,due,status,received_at) "
        ...
        t.check(
            bool(_TS_RE.match(after["reported_at"] or "")), f"in_progress stamp well-formed i={i}"
        )
    ...
    for status in ("reported", "delivered", "voided", "cancelled"):
        pid = con.execute(
            "INSERT INTO patients(name,age,age_desc,sex,telephone) "
            "VALUES('Other',30,'Years','Male','03001234567')"
        ).lastrowid
        rid = con.execute(
            "INSERT INTO receipts(lab_no,patient_id,patient_name,age,age_desc,sex,"
        ...
        t.eq(after["status"], status, f"non-pending status '{status}' unchanged by finalisation")
    ...
    for recv, rep in REP_DATES:
        ...
        con.execute("UPDATE receipts SET received_at=?, reported_at=? WHERE id=?", (recv, rep, rid))
        con.commit()
        rh, err = _safe_report_html(t, rid)
    ...
    for recv in ["2026-05-05 10:00", "2020-01-01 00:00", "garbage-date", ""]:
        ...
        con.execute("UPDATE receipts SET received_at=?, reported_at=NULL WHERE id=?", (recv, rid))
        con.commit()
        rh, err = _safe_report_html(t, rid)
    ...
    for recv, rep in [
        ...
        if ch is None:
            continue
        t.check(
            "reporting date" not in ch.lower(), f"cash receipt omits Reporting Date recv={recv!r}"
        )
        ...
        try:
            ...
            cb, errc = None, repr(e)
        t.check(errc is None and _is_pdf(cb), f"cash receipt PDF builds recv={recv!r} err={errc}")
    ...
    for recv, year in [
        ("2026-06-01 09:00", "2026"),
        ("2019-01-01 00:00", "2019"),
        ("1999-12-31 23:59", "1999"),
        ("2100-05-05 05:05", "2100"),
    ]:
        rid = t.make_receipt(
            "03004445555",
            sub=200,
