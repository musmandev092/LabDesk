...
def register(t):
    ...
    PWS = [
        "", " ", "a", "ab", "abc", "admin", "password", "Password1",
        "s3cret-pw", "oldpw", "12345678", "0000", "correct horse battery",
        "p@$$w0rd!", "with$dollar$signs", "scrypt$fake$1$1$salt$hash",
        "tabs\tand\nnewlines", "trailing ", " leading", "  double  ",
        "UPPER", "MiXeD", "ünïcödé", "密码口令", "emoji😀pw", "résumé",
        "a" * 1, "a" * 64, "a" * 256, "a" * 1024,
        "null\x00byte", "líne", "Ω≈ç√∫", "123456789012345678901234567890",
        "P@ssw0rd", "qwerty", "letmein", "Sargodha-Lab-2026",
    ]
    ...
    for pw in PWS:
        ...
        t.check(all(c in "0123456789abcdef" for c in salt),
                f"salt is lowercase hex pw={pw!r}")
        t.check(all(c in "0123456789abcdef" for c in hexh),
                f"hash is lowercase hex pw={pw!r}")
    ...
    for i, target in enumerate(pw_list):
        ...
        for mut in (target + " ", " " + target, target + "x", target.upper() + "_"):
            if mut == target:
                continue
            t.check(db._verify_password(mut, h, "") is False,
                    f"reject near-miss {mut!r} vs {target!r}")
    ...
    for pw in ["admin", "password", "s3cret-pw", "", "密码口令"]:
        for salt in ["00" * 16, "deadbeef" * 4, "ab" * 16, "f" * 32]:
            ...
            t.check(db._verify_password(pw, h1, "") is True,
    ...
    LEGACY_SALTS = ["", "abc123", "saltysalt", "0", "Z" * 40, "deadbeef",
                    "  spaces  ", "$pecial$", "1234567890", "Ω≈ç"]
    LEGACY_PWS = ["oldpw", "admin", "password", "", "12345678", "ünïcödé",
                  "P@ssw0rd", "qwerty", "密码口令", "a" * 128, " ", "x"]
    for pw in LEGACY_PWS:
        for ls in LEGACY_SALTS:
            digest = _sha256(ls, pw)
            t.check(db._verify_password(pw, digest, ls) is True,
                    f"legacy sha256 verifies pw={pw!r} salt={ls!r}")
            # wrong salt -> reject (unless salts collide, which they won't here)
            wrong_salt = ls + "x"
            t.check(db._verify_password(pw, digest, wrong_salt) is False,
                    f"legacy wrong salt rejected pw={pw!r}")
            # wrong password -> reject
            t.check(db._verify_password(pw + "!", digest, ls) is False,
    ...
    for ghost in ["ghost", "", "nobody", "Robert'); DROP TABLE users;--", "admin "]:
        t.check(db.verify_user(con, ghost, "x") is None,
                f"unknown user rejected user={ghost!r}")
    ...
    for uname, pw in USERS:
        ...
        t.check(db.verify_user(con, uname, pw + "X") is None,
                f"wrong pw rejected user={uname}")
        t.check(db.verify_user(con, uname, "") is None,
                f"empty pw rejected user={uname}")
        # failures were counted (sub-threshold, so no lock yet)
        rr = con.execute("SELECT failed_attempts, locked_until FROM users WHERE username=?",
                         (uname,)).fetchone()
    ...
    t.check(db.verify_user(con, "op_inactive", "right-pw") is None,
    ...
    for i, (uname, pw, ls) in enumerate([
        ...
        post = con.execute("SELECT pass_hash, salt FROM users WHERE username=?",
                          (uname,)).fetchone()
        t.check(post["pass_hash"].startswith("scrypt$"),
                f"legacy hash upgraded to scrypt user={uname}")
        t.eq(post["salt"], "", f"legacy salt cleared after upgrade user={uname}")
        # and the new scrypt hash still verifies the same password
        t.check(db._verify_password(pw, post["pass_hash"], "") is True,
                f"upgraded scrypt verifies user={uname}")
        # subsequent login still works (now via scrypt path)
        t.check(db.verify_user(con, uname, pw) is not None,
                f"re-login works post-upgrade user={uname}")
    ...
    def _reset(uname):
        con.execute("UPDATE users SET failed_attempts=0, locked_until=NULL WHERE username=?",
                    (uname,))
        con.commit()
    ...
    for attempt in range(1, MAX):
        ...
        fa = con.execute("SELECT failed_attempts FROM users WHERE username='lock_user'"
                         ).fetchone()[0]
        t.eq(fa, attempt, f"failed_attempts increments to {attempt}")
        lu = con.execute("SELECT locked_until FROM users WHERE username='lock_user'"
                        ).fetchone()[0]
        t.check(lu is None, f"not locked before threshold attempt={attempt}")
        t.eq(db.lock_remaining(con, "lock_user"), 0,
    ...
    t.check(db.verify_user(con, "lock_user", "good-pw") is None,
            "correct pw refused while locked")
    ...
    for extra in range(0, 12):
        ...
        con.execute("UPDATE users SET failed_attempts=?, locked_until=NULL "
                    "WHERE username='lock_user'", (fa_val - 1,))
        con.commit()
        db.verify_user(con, "lock_user", "bad")
        row = con.execute("SELECT failed_attempts, locked_until FROM users "
                         "WHERE username='lock_user'").fetchone()
        ...
        t.check(rem >= min(expected, LOCK_MAX) - 5,
    ...
    t.check(db.verify_user(con, "lock_user", "good-pw") is not None,
            "login works again after reset")
    # ...and a successful login itself zeroes the counters
    con.execute("UPDATE users SET failed_attempts=3, locked_until=NULL "
                "WHERE username='lock_user'")
    con.commit()
    t.check(db.verify_user(con, "lock_user", "good-pw") is not None, "login with sub-threshold fails")
    rr = con.execute("SELECT failed_attempts, locked_until FROM users WHERE username='lock_user'"
                     ).fetchone()
    ...
    for dt in [1, 2, 5, 10, 15, 30, 45, 60, 90, 120, 240, 480, 600,
               900, 1200, 1800, 2400, 3000, 3600, 7200]:
        cases.append((str(now + dt), dt, f"future +{dt}s"))
    # garbage strings -> 0
    for g in ["", "   ", "null", "None", "NaN_x", "12.34.56", "0x10", "++3",
              "3,600", "1_000", "sixty", "60s", "@#$", "true"]:
    ...
    for pw in ["small-wf", "tiny"]:
        salt = "11" * 16
        small = hashlib.scrypt(pw.encode(), salt=salt.encode(), n=1024, r=8, p=1,
                               maxmem=db._SCRYPT_MAXMEM, dklen=32)
        sstr = f"scrypt$1024$8$1${salt}${small.hex()}"
        t.check(db._verify_password(pw, sstr, "") is True,
                f"custom work-factor scrypt verifies pw={pw!r}")
        t.check(db._verify_password(pw + "x", sstr, "") is False,
                f"custom work-factor wrong pw rejected pw={pw!r}")
        # shorter dklen honoured (len(hexh)//2)
        short = hashlib.scrypt(pw.encode(), salt=salt.encode(), n=1024, r=8, p=1,
                               maxmem=db._SCRYPT_MAXMEM, dklen=16)
        sstr16 = f"scrypt$1024$8$1${salt}${short.hex()}"
        t.check(db._verify_password(pw, sstr16, "") is True,
                f"16-byte dklen scrypt verifies pw={pw!r}")
