...
# ---------------------------------------------------------------------------
def _mk_receipt(con, *, status="pending", sex="Male", phone="03001234567", reported_at=None):
    ...
    test_id = con.execute(
        "SELECT test_id FROM test_parameters GROUP BY test_id " "ORDER BY COUNT(*) DESC LIMIT 1"
    ).fetchone()[0]
...
def register(t):
    ...
    for start in ["pending", "in_progress"]:
        ...
        _finalise(con, rid)
        row = con.execute("SELECT status, reported_at FROM receipts WHERE id=?", (rid,)).fetchone()
    ...
    for start in ["pending", "in_progress"]:
        rid, item_id, _ = _mk_receipt(con, status=start)
        _finalise(con, rid)
        first = con.execute("SELECT reported_at FROM receipts WHERE id=?", (rid,)).fetchone()[
            "reported_at"
        ]
        # tamper to a sentinel, re-finalise: WHERE status IN(pending,in_progress)
        # no longer matches, so the sentinel must survive
        con.execute("UPDATE receipts SET reported_at=? WHERE id=?", ("1999-12-31 23:59", rid))
        con.commit()
        _finalise(con, rid)
        again = con.execute(
    ...
    params = con.execute(
        ...
        "AND name IS NOT NULL AND name<>'' ORDER BY seq LIMIT 12",
        (test_id,),
    ).fetchall()
    ...
    for p in params:
        for i, v in enumerate(VALUES):
            ...
            t.eq(row["ref_text"], resolve_ref(p, "Male"), f"ref_text snapshot p={p['id']}")
    ...
    t.eq(
        con.execute(
            "SELECT value FROM results WHERE receipt_item_id=? AND parameter_id=?", (item_id, pid0)
        ).fetchone()["value"],
        "FINAL",
    ...
    t.eq(
        con.execute(
            "SELECT hidden FROM results WHERE receipt_item_id=? AND parameter_id=?", (item_id, pid0)
        ).fetchone()["hidden"],
        1,
        "hidden set to 1",
    )
    ...
    params_f = con.execute(
        "SELECT * FROM test_parameters WHERE test_id=? AND part_type='N' "
        "AND name IS NOT NULL AND name<>'' ORDER BY seq LIMIT 8",
        (test_f,),
    ).fetchall()
    for p in params_f:
    ...
    for i, v in enumerate(VALUES + ["overwrite", "", "again"]):
        ...
        rows = con.execute(
            ...
            "WHERE receipt_item_id=? AND parameter_id IS NULL",
            (item_s,),
        ).fetchall()
    ...
    pset = con.execute(
        ...
        "AND name IS NOT NULL AND name<>'' ORDER BY seq LIMIT 6",
        (test_u,),
    ).fetchall()
    for pr in pset:
    ...
    for pr in pset:
        t.eq(
            con.execute(
                "SELECT value FROM results WHERE receipt_item_id=? " "AND parameter_id=?",
                (item_u, pr["id"]),
            ).fetchone()["value"],
            "v2",
            f"second pass updated value p={pr['id']}",
        )
    ...
    con.execute(
        "INSERT INTO results(receipt_item_id,parameter_id,value) VALUES(?,?,?)", (item_x, px, "a")
    )
    con.commit()
    raised = False
    try:
    ...
    for sex in ["Male", "Female"]:
        for start in ["pending", "in_progress"]:
            ...
            pw = con.execute(
                "SELECT * FROM test_parameters WHERE test_id=? AND part_type='N' "
                "AND name IS NOT NULL AND name<>'' ORDER BY seq LIMIT 5",
                (test_w,),
            ).fetchall()
            for p in pw:
            ...
            _finalise(con, rid_w)
            r = con.execute(
                "SELECT status, reported_at FROM receipts WHERE id=?", (rid_w,)
            ).fetchone()
            t.eq(r["status"], "reported", f"finalise flow status sex={sex} start={start}")
            t.check(
                r["reported_at"] is not None, f"finalise flow reported_at sex={sex} start={start}"
            )
            nres = con.execute(
                "SELECT COUNT(*) FROM results WHERE receipt_item_id=?", (item_w,)
            ).fetchone()[0]
            ...
            for p in pw:
                t.eq(
                    ...
                    "7",
                    f"value persisted sex={sex} p={p['id']}",
                )
    ...
    for st in ["pending", "in_progress", "reported", "delivered"]:
        pid = con.execute(
            "INSERT INTO patients(name,sex) VALUES(?,?)", (f"{tag} {st}", "Male")
        ).lastrowid
        rid = con.execute(
    ...
    vpid = con.execute(
        "INSERT INTO patients(name,sex) VALUES(?,?)", (f"{tag} void", "Male")
    ).lastrowid
    vrid = con.execute(
    ...
    def _list(status):
        sql = (
            f"SELECT id FROM receipts WHERE {t.db.NOT_VOIDED} "
    ...
    for st in ["pending", "in_progress", "reported", "delivered"]:
        ...
        for other in ["pending", "in_progress", "reported", "delivered"]:
            if other != st:
                t.check(made[other] not in ids, f"list status={st} excludes status={other}")
    all_ids = _list("All")
    for st in ["pending", "in_progress", "reported", "delivered"]:
    ...
    for txt, want in [
        ("Hemolysed sample", "Hemolysed sample"),
        ("", None),
        ("   ", None),
        ("ok", "ok"),
        ("x" * 500, "x" * 500),
        ...
        norm = txt.strip()
        con.execute("UPDATE receipt_items SET remarks=? WHERE id=?", (norm or None, item_r))
        con.commit()
        got = con.execute("SELECT remarks FROM receipt_items WHERE id=?", (item_r,)).fetchone()[
            "remarks"
        ]
