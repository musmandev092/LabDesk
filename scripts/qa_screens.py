...
from labdesk.presentation.style import apply_theme
...
cbc = con.execute(
    "SELECT id,name,charges FROM tests WHERE name LIKE '%CBC%' LIMIT 1"
).fetchone()
lft = con.execute(
    "SELECT id,name,charges FROM tests WHERE name LIKE '%LFT%' LIMIT 1"
).fetchone()
cult = con.execute(
    "SELECT id,name,charges FROM tests WHERE is_culture=1 LIMIT 1"
).fetchone()
r1, i1 = make_receipt(
    "Muhammad Usman",
    "Mr.",
    ...
    35,
    [
        (cbc["id"], cbc["name"], cbc["charges"]),
        (lft["id"], lft["name"], lft["charges"]),
    ],
    500,
)
r2, _ = make_receipt(
...
if cult:
    r3, _ = make_receipt(
        ...
        "Male",
        40,
        [(cult["id"], cult["name"], cult["charges"])],
        0,
        "pending",
    )
...
def render_theme(theme: str):
    ...
    # pre-login surfaces
    from labdesk.presentation.setup_wizard import SetupWizard

    shot(SetupWizard(con), "01_setup_wizard", 720, 800)
    from labdesk.presentation.login import LoginDialog

    shot(LoginDialog(con), "02_login", 460, 620)

    # main window — every page by label
    from labdesk.presentation.main_window import MainWindow
    ...
    # dialogs -----------------------------------------------------------------
    # (these classes moved into sibling *_dialogs modules during the Wave 6 decomposition)
    ...
    from labdesk.presentation.doctors import DoctorDialog

    drow = con.execute("SELECT * FROM doctors LIMIT 1").fetchone()
    shot(DoctorDialog(None, drow), "21_doctor_dialog", 440, 380)
    from labdesk.presentation.receipt_dialogs import _EditReceiptDialog
    ...
    from labdesk.presentation.settings_dialogs import UserDialog
