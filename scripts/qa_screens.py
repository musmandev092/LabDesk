...
r1, i1 = make_receipt("Muhammad Usman","Mr.","Male",35,[(cbc['id'],cbc['name'],cbc['charges']),(lft['id'],lft['name'],lft['charges'])],500)
r2, _ = make_receipt("Ayesha Bibi","Mrs.","Female",28,[(cbc['id'],cbc['name'],cbc['charges'])],300,"reported")
if cult:
...
con.execute("UPDATE receipts SET reported_at=datetime('now','localtime'), status='reported' WHERE id=?", (r2,))
...
def render_theme(theme: str):
    ...
    win = MainWindow(con, user); win.resize(1320, 860); win.show(); settle()
    for label, name in PAGES:
        win.go(win._page_index[label]); settle()
        win.grab().save(str(outdir / f"{name}.png")); print(f"  [{theme}] {name}")

    # filled / selected states ------------------------------------------------
    # reception filled
    win.go(win._page_index["Reception / Billing"]); rec = win.pages[win._page_index["Reception / Billing"]]
    ...
    rec.search_tests("CBC"); settle()
    if rec.results.count():
        rec.add_from_list(rec.results.item(0))
    rec.search_tests("LFT"); settle()
    if rec.results.count():
        rec.add_from_list(rec.results.item(0))
    rec.paid.setValue(400); settle()
    win.grab().save(str(outdir / "04b_reception_filled.png")); print(f"  [{theme}] 04b_reception_filled")

    # receipts with a row selected (toolbar lights up)
    win.go(win._page_index["Receipts / Reports"]); rcpt = win.pages[win._page_index["Receipts / Reports"]]
    settle()
    if rcpt.table.rowCount():
        rcpt.table.selectRow(0); settle()
    win.grab().save(str(outdir / "05b_receipts_selected.png")); print(f"  [{theme}] 05b_receipts_selected")

    # worklist result entry
    win.go(win._page_index["Worklist / Results"]); wl = win.pages[win._page_index["Worklist / Results"]]
    wl.status_filter.setCurrentIndex(0); settle()
    if wl.table.rowCount():
        wl.table.selectRow(0); wl.load_receipt(); settle()
    win.grab().save(str(outdir / "06b_worklist_results.png")); print(f"  [{theme}] 06b_worklist_results")

    # catalog params
    win.go(win._page_index["Test Catalog"]); cat = win.pages[win._page_index["Test Catalog"]]
    settle()
    ...
    settle()
    win.grab().save(str(outdir / "07b_catalog_params.png")); print(f"  [{theme}] 07b_catalog_params")

    # microbiology selected
    win.go(win._page_index["Microbiology"]); mb = win.pages[win._page_index["Microbiology"]]
    settle()
    if mb.table.rowCount():
        ...
        if hasattr(mb, "load_item"):
            mb.load_item()
        settle()
    win.grab().save(str(outdir / "09b_microbiology_sel.png")); print(f"  [{theme}] 09b_microbiology_sel")

    # accounts tabs
    win.go(win._page_index["Accounts"]); ac = win.pages[win._page_index["Accounts"]]
    settle()
    for ti, nm in [(1, "10b_accounts_expenses"), (2, "10c_accounts_dues")]:
        if ac.tabs.count() > ti:
            ac.tabs.setCurrentIndex(ti); settle()
            win.grab().save(str(outdir / f"{nm}.png")); print(f"  [{theme}] {nm}")
