...
def make_receipt(name, sex, age, tests, paid, status="pending"):
    pid = con.execute(
        "INSERT INTO patients(mr_no,name,age,age_desc,sex,telephone) VALUES (?,?,?,?,?,?)",
        ("", name, age, "Years", sex, "03001234567"),
    ).lastrowid
    mr = f"MR{pid:05d}"
    con.execute("UPDATE patients SET mr_no=? WHERE id=?", (mr, pid))
    sub = sum(t[2] for t in tests)
    rid = con.execute(
        """INSERT INTO receipts(lab_no,case_no,mr_no,patient_id,doctor_id,patient_name,age,age_desc,sex,
...
for w, h, tag in SIZES:
    ...
    print(
        f"  [{tag}] rendered {len(PAGES)} pages -> actual {win.width()}x{win.height()}"
    )
    win.close()
